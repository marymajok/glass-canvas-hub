...
const ClientDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    setStats({ totalBookings: bookingsData?.length || 0, pendingBookings: bookingsData?.filter(b => b.status === "pending").length || 0, completedBookings: bookingsData?.filter(b => b.status === "completed").length || 0, reviewsGiven: reviewsCount || 0 });
    setLoading(false);
  };
